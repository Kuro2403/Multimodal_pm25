...
def load_data(datadir, samples_file):
    ...
    try:
        ...
        for station in tqdm(samples_df.AirQualityStation.unique()):
            ...
            # fp = os.path.join(datadir, "sentinel-5p", s5p_path)
            # s5p_data = xr.open_dataset(fp, engine="netcdf4").rio.write_crs(4326)
            fp = os.path.join(datadir, "sentinel-5p", s5p_path)
            try:
                s5p_data = xr.open_dataset(fp, engine="netcdf4").rio.write_crs(4326)
            except Exception:
                s5p_data = xr.open_dataset(fp, engine="h5netcdf").rio.write_crs(4326)
