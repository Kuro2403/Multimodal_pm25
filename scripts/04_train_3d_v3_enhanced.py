...
class R2Plus1DBlock(nn.Module):
    """Pseudo-3D (R(2+1)D) Block: Tách biệt Không gian (Spatial) và Thời gian (Temporal)
       Giảm tham số, chống overfit cực tốt cho dữ liệu vệ tinh nhỏ."""
...
class BNNIN3D(nn.Module):
    def __init__(self, out_dim=CNN_DIM):
        ...
        # Nâng cấp lớp Head: Bổ sung LayerNorm và Dropout để ổn định đặc trưng
        self.fc   = nn.Sequential(
            nn.Flatten(), 
...
def train_neural(model, x_seq_tr, x_ss_tr, x_bp_tr, x_ctx_tr, y_std_tr,
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        ...
        for xs, xss, xb, xc, yb in tr_ld:
            # Data Augmentation: Bơm nhiễu ngẫu nhiên để mô hình chống overfit
...
def make_gbr(): return GradientBoostingRegressor(
    n_estimators=1500, max_depth=4, learning_rate=0.015, subsample=0.8,
...
def make_rf(): return RandomForestRegressor(
    n_estimators=1000, max_depth=7, min_samples_leaf=2,
    max_features=0.7, random_state=42, n_jobs=-1)
    
def make_knn(): return KNeighborsRegressor(n_neighbors=12, weights='distance')
...
def make_plots(y_va, y_te, preds_dict, out_dir):
    ...
    plt.suptitle(
        "train_3d_v3: BiLSTM + 3D BN-NIN + OOF Ridge  |  preprocessed daily_merged.csv",
...
def main():
    ...
    SEP = "=" * 78
    print(SEP)
    print("  train_3d_v3: BiLSTM + 3D BN-NIN + OOF ElasticNet Stacking")
    ...
    # Giải pháp quan trọng: Trở lại sử dụng np.log1p thuần túy thay vì QuantileTransformer.
    # Tree models (XGBoost, RF...) hoạt động cực kỳ ổn định với log1p nhưng lại dễ bị nội suy sai 
    # ở các giá trị cực trị (spikes) nếu dùng QuantileTransformer.
    ...
    # Nới lỏng một chút: chỉ giảm trọng số cho PM2.5 > 100 để không bỏ lỡ các đợt ô nhiễm nặng
    ...
    # ── Bypass & context ──────────────────────────────────────────────────────
    # Mở rộng bypass connection để NN hưởng lợi từ các tính năng thống kê mạnh nhất
    bp_feats = [c for c in ["pm25_lag1", "pm25_lag2", "pm25_lag3", "pm25_lag7",
                            "pm25_roll3", "pm25_roll7", "pm25_trend_7",
                            "pm25_spatial_mean", "pm25_spatial_lag1", "pm25_spatial_roll7",
    ...
    # Trở lại 8 folds vì 10 folds gây bất ổn do kích thước tập validation quá nhỏ
    ...
    for fold, (fi_tr, fi_va) in enumerate(kf_full.split(Xfull)):
        ...
        oof_rf[fi_va]  = inv_log(m.predict(Xfull[fi_va]))
        m = make_knn(); m.fit(Xfull_sc[fi_tr], yfull_fit[fi_tr])
        oof_knn[fi_va] = inv_log(m.predict(Xfull_sc[fi_va]))
        m = make_hgb(); m.fit(Xfull[fi_tr], yfull_fit[fi_tr])
        oof_hgb[fi_va] = inv_log(m.predict(Xfull[fi_va]))
        print("done")

    oof_stack = np.column_stack([oof_xgb, oof_lgb, oof_gbr, oof_cat, oof_etr, oof_rf, oof_knn, oof_hgb])
    
    # Giải pháp cải tiến: Sử dụng Non-Negative Linear Regression
    # Việc ràng buộc trọng số dương (NNLS) giúp Meta-Learner cực kỳ ổn định,
    # loại bỏ hoàn toàn hiện tượng overfit của LightGBM và ensemble hoàn hảo 6 mô hình.
    meta_model = LinearRegression(positive=True)
    meta_model.fit(oof_stack, y_raw_full)
    print(f"  Meta-learner (NNLS LinearRegression) trained.")
    print(f"  Weights: XGB={meta_model.coef_[0]:.3f} LGB={meta_model.coef_[1]:.3f} GBR={meta_model.coef_[2]:.3f} CAT={meta_model.coef_[3]:.3f} ETR={meta_model.coef_[4]:.3f} RF={meta_model.coef_[5]:.3f} KNN={meta_model.coef_[6]:.3f} HGB={meta_model.coef_[7]:.3f}")
    ...
    # Ép GBR học trên toàn bộ Xfull (Tránh hụt mất 15% dữ liệu Validation quan trọng)
    gbrm = GradientBoostingRegressor(n_estimators=1000, max_depth=4, learning_rate=0.015, 
    ...
    hgbm = make_hgb(); hgbm.fit(Xfull, yfull_fit)

    def tree_pred(X, X_s):
        raw = np.column_stack([
            inv_log(xgbm.predict(X)), inv_log(lgbm.predict(X)),
            inv_log(gbrm.predict(X)), inv_log(catm.predict(X)),
            inv_log(etrm.predict(X)), inv_log(rfm.predict(X)),
            inv_log(knnm.predict(X_s)), inv_log(hgbm.predict(X))])
