...
def prepare_data():
    ...
    # 3. Data Split
    test_dates = df['date'].dt.year >= 2023
    val_dates = (df['date'].dt.year == 2022) & (df['date'].dt.month >= 7)
    train_dates = ~test_dates
    
    tr_idx = np.where(train_dates)[0]
    va_idx = np.where(val_dates)[0]  # Not used for fitting anymore
...
def get_xgb_objective(X_tr, y_tr, X_va, y_va):
    def objective(trial):
        params = {
...
def get_lgb_objective(X_tr, y_tr, X_va, y_va):
    def objective(trial):
        params = {
            'n_estimators': trial.suggest_int('n_estimators', 200, 1000),
            'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
...
def get_rf_objective(X_tr, y_tr, X_va, y_va):
    def objective(trial):
        params = {
...
if __name__ == "__main__":
    seed_everything(42)
    X_tr, X_va, X_te, y_tr, y_va, y_te = prepare_data()
    
    N_TRIALS = 30
    
    print("\\n--- Tuning XGBoost ---")
    xgb_study = optuna.create_study(direction='maximize')
    xgb_study.optimize(get_xgb_objective(X_tr, y_tr, X_va, y_va), n_trials=N_TRIALS)
    print(f"Best XGBoost Val R2: {xgb_study.best_value:.4f}")
    
    print("\\n--- Tuning LightGBM ---")
    lgb_study = optuna.create_study(direction='maximize')
    lgb_study.optimize(get_lgb_objective(X_tr, y_tr, X_va, y_va), n_trials=N_TRIALS)
    print(f"Best LightGBM Val R2: {lgb_study.best_value:.4f}")
    
    print("\\n--- Tuning Random Forest ---")
    rf_study = optuna.create_study(direction='maximize')
    rf_study.optimize(get_rf_objective(X_tr, y_tr, X_va, y_va), n_trials=N_TRIALS)
    print(f"Best Random Forest Val R2: {rf_study.best_value:.4f}")
    
    # Train final models with best params
    print("\\n--- Training Final Models ---")
    xgb_model = xgb.XGBRegressor(**xgb_study.best_params, random_state=42, n_jobs=-1)
    xgb_model.fit(np.vstack((X_tr, X_va)), np.concatenate((y_tr, y_va)))
    xgb_te_preds = xgb_model.predict(X_te)
    print(f"XGBoost Test R2: {r2_score(y_te, xgb_te_preds):.4f}")
    
    lgb_model = lgb.LGBMRegressor(**lgb_study.best_params, random_state=42, n_jobs=-1)
    lgb_model.fit(np.vstack((X_tr, X_va)), np.concatenate((y_tr, y_va)))
    lgb_te_preds = lgb_model.predict(X_te)
    print(f"LightGBM Test R2: {r2_score(y_te, lgb_te_preds):.4f}")
    
    rf_model = RandomForestRegressor(**rf_study.best_params, random_state=42, n_jobs=-1)
    rf_model.fit(np.vstack((X_tr, X_va)), np.concatenate((y_tr, y_va)))
    rf_te_preds = rf_model.predict(X_te)
    print(f"Random Forest Test R2: {r2_score(y_te, rf_te_preds):.4f}")
    
    # Meta-Model Stacking
    print("\\n--- Training Meta-Model (Ridge Stacking) ---")
    xgb_va_preds = xgb.XGBRegressor(**xgb_study.best_params, random_state=42, n_jobs=-1).fit(X_tr, y_tr).predict(X_va)
    lgb_va_preds = lgb.LGBMRegressor(**lgb_study.best_params, random_state=42, n_jobs=-1).fit(X_tr, y_tr).predict(X_va)
    rf_va_preds = RandomForestRegressor(**rf_study.best_params, random_state=42, n_jobs=-1).fit(X_tr, y_tr).predict(X_va)
    
    X_meta_va = np.column_stack((xgb_va_preds, lgb_va_preds, rf_va_preds))
    meta_model = Ridge(alpha=1.0)
    meta_model.fit(X_meta_va, y_va)
    
    X_meta_te = np.column_stack((xgb_te_preds, lgb_te_preds, rf_te_preds))
    meta_te_preds = meta_model.predict(X_meta_te)
    final_r2 = r2_score(y_te, meta_te_preds)
