...
RAW_MERGED_FILE = ROOT / "data/processed/01_daily_merged_clean.csv"
...
# ]
# Mới (Đã sửa theo data CSV thực tế)
MET_COLS = [
...
# -----------------------------------------------------------------------------
# PYTORCH LSTM & MULTIMODAL BASELINE MODELS
# -----------------------------------------------------------------------------
SEQ_LEN = 7
...
def impute_satellite(df: pd.DataFrame, sat_cols: list, tr_idx: list, method="interpolate") -> pd.DataFrame:
    ...
    # Create missingness indicators FIRST
    for c in sat_cols:
        if c in df.columns:
...
# -----------------------------------------------------------------------------
def run_ablation_pipeline(drop_cols=[], outlier_filt=True):
    seed_everything(42)
    ...
    # Pad satellite columns
    for c in SAT_COLS_3D:
        if c not in df.columns:
            df[c] = 0.0
    # 1. (Moved Spatial Logic below split)
    # 2. Split Data
    ...
    # 4. Outlier Filtering
    if outlier_filt and "pm25_count" in df.columns:
        tr_idx = [i for i in tr_idx_full if df.loc[i, "pm25_count"] >= 3]
    else:
        tr_idx = tr_idx_full
    # 4.5. New K-NN Spatial & Baseline Features
    ...
    if len(stations) > 2:
        ...
        for i, row in enumerate(indices):
            ...
            # Exclude self (row[0]), take up to 2 neighbors
        ...
        # Fill missing KNN with overall spatial mean for the day to prevent NaNs
    ...
    for loc_id, grp in df.groupby("location_id"):
        ...
        # 5.5. Smooth TROPOMI Features (Rolling 7-day to remove daily noise)
        for c in SAT_COLS_3D:
            if c in grp.columns:
        ...
        for c in AOD_COLS:
            if c in grp.columns:
                df.loc[grp.index, f"{c}_roll7"] = grp[c].rolling(window=7, min_periods=1).mean()
    # 5.6 Safe Imputation for Satellite Features (Zero Leakage)
    # We avoid .interpolate(linear) because it peeks into the future.
    print("Safely Imputing Satellite Features...")
    for c in SAT_COLS_3D:
        if c in df.columns:
            # 1. Fill with Spatial Mean for the day (if other stations have data)
            daily_mean = df.groupby("date")[c].transform("mean")
            df[c] = df[c].fillna(daily_mean)
            
            # 2. Fill remaining with Temporal Forward Fill (ffill) per location (uses only past data)
            df[c] = df.groupby("location_id")[c].transform(lambda x: x.ffill())
            
            # 3. Backfill only if the very first days are NaN
            df[c] = df.groupby("location_id")[c].transform(lambda x: x.bfill())
    # Compute Raw Lags
    for lag in [1, 2, 3, 7]:
        df[f"pm25_lag{lag}"] = df.groupby("location_id")["pm25"].transform(lambda s: s.shift(lag))
        
    # Satellite Anomalies & Lags
    ...
    # 6. Physics Interactions (Ventilation Coefficient)
    if "wind_speed_mean_kmh" in df.columns and "blh_mean_m" in df.columns:
        ...
        PHYSICS_COLS = []
    # Feature List
    ADVANCED_LAGS = ["pm25_ema7", "pm25_ema15", "pm25_std7"]
    MISSING_FLAGS = [f"{c}_missing" for c in SAT_COLS_3D]
    
    MISSING_FLAGS = [f"{c}_missing" for c in SAT_COLS_3D]
    
    SAT_ROLLING = [f"{c}_roll7" for c in SAT_COLS_3D + AOD_COLS]
    SAT_ANOMALIES = [f"{c}_anomaly" for c in SAT_COLS_3D + AOD_COLS]
    SAT_LAGS = [f"{c}_lag1" for c in SAT_COLS_3D + AOD_COLS]
    
    features_tab = (
        MET_COLS + AOD_COLS + BLH_COLS + FRP_COLS + TIME_COLS + PHYSICS_COLS +
        ["pm25_lag1", "pm25_lag2", "pm25_lag3", "pm25_lag7", "pm25_spatial_lag1", "station_baseline_pm25"] +
    ...
    features_tab = [f for f in features_tab if f in df.columns and f not in drop_cols]
    features_tab = list(dict.fromkeys(features_tab)) # Deduplicate
    # Standardize Tabular Features for ML
    ...
    # Satellite columns for DL
    df_sat_sc = df.copy()
    sat_cols_dl = [f for f in SAT_COLS_3D if f in df.columns and f not in drop_cols]
    ...
    lstm_preds = y_scaler.inverse_transform(np.concatenate(lstm_preds_te)).ravel()
    lstm_r2 = r2_score(y_te, lstm_preds)
    
    # E. Lightweight Multimodal
    if len(sat_cols_dl) > 0:
        ...
        mm_preds = y_scaler.inverse_transform(np.concatenate(mm_preds_te)).ravel()
        mm_r2 = r2_score(y_te, mm_preds)
    else:
        mm_r2 = lstm_r2
...
# -----------------------------------------------------------------------------
def main():
    print("Starting Comprehensive Ablation Study across models...")
    ...
    configs = {
        "C1 (Baseline)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1"],
            "outlier_filt": True
        },
        "F1 (+ Atmospheric Stagnation)": {
            "drop_cols": ["rh_aod_interaction", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1"],
            "outlier_filt": True
        },
        "F2 (+ RH x AOD)": {
            "drop_cols": ["stagnation_index", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1"],
            "outlier_filt": True
        },
        "F3 (+ Lag Decay)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1"],
            "outlier_filt": True
        },
        "F4 (+ Rolling Max/Median)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_lag_decay", "episode_length_lag1"],
            "outlier_filt": True
        },
        "F5 (+ Episode Length)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median"],
            "outlier_filt": True
        },
        "C2 (All New Features)": {
            "drop_cols": [],
    ...
    for config_name, params in configs.items():
        ...
        t0 = time.time()
        scores = run_ablation_pipeline(**params)
        print(f"Completed in {time.time() - t0:.2f} seconds.")
        
        for model_name, r2_score_val in scores.items():
            results.append({
                "Configuration": config_name,
                "Model": model_name,
                "Test R2": round(r2_score_val, 4)
            })
            
    df_results = pd.DataFrame(results)
    
    # Pivot results for visual presentation
    df_pivot = df_results.pivot(index="Configuration", columns="Model", values="Test R2")
    df_pivot = df_pivot.reindex(list(configs.keys()))
    
    # Save CSV
    try:
        df_pivot.to_csv(OUTPUT_DIR / "comprehensive_ablation_table.csv")
    except PermissionError:
        print(f"\n[Warning] Could not save to {OUTPUT_DIR / 'comprehensive_ablation_table.csv'} (Permission denied).")
    
    print("\n=== COMPREHENSIVE ABLATION STUDY RESULTS (Test R2) ===")
    
    headers = ["Configuration"] + list(df_pivot.columns)
    md_table = "| " + " | ".join(headers) + " |\n"
    md_table += "| " + " | ".join(["---"] * len(headers)) + " |\n"
    for idx, row in df_pivot.iterrows():
        md_table += f"| {idx} | " + " | ".join(str(val) for val in row) + " |\n"
    print(md_table)
    
    # Plot Comparison Chart
    plt.figure(figsize=(12, 7))
    df_pivot.plot(kind="bar", edgecolor="black", alpha=0.85, width=0.8, ax=plt.gca())
    plt.title("Ablation Study: Test R2 comparison across models", pad=15, fontweight="bold")
    ...
    try:
        plt.savefig(OUTPUT_DIR / "comprehensive_ablation_comparison.png", dpi=300)
        print(f"\nSaved comprehensive ablation chart to: {OUTPUT_DIR / 'comprehensive_ablation_comparison.png'}")
    except PermissionError:
        print(f"\n[Warning] Could not save chart to {OUTPUT_DIR / 'comprehensive_ablation_comparison.png'} (Permission denied).")
