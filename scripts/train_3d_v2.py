...
from sklearn.linear_model import Ridge, LinearRegression, ElasticNet, RidgeCV
...
OUT_DIR   = ROOT / "outputs/final_3d_v2"
...
def make_plots(y_va, y_te, preds_dict, out_dir):
    ...
    plt.suptitle(
        "train_3d_v2: BiLSTM + 3D BN-NIN + OOF Stacking  |  preprocessed daily_merged.csv",
...
def main():
    ...
    SEP = "=" * 78
    print(SEP)
    print("  train_3d_v2: BiLSTM + 3D BN-NIN + OOF ElasticNet Stacking")
    ...
    # QuantileTransformer on full train target (no clip).
    # n_quantiles=500 for smoother mapping; subsample all train rows.
    y_sc = QuantileTransformer(output_distribution="normal", n_quantiles=500,
                               subsample=len(tr_idx), random_state=42)
    ...
    # Conditional Stacking: Add meta-features to the meta-learner's input
    ...
    meta_model = lgb.LGBMRegressor(n_estimators=50, max_depth=2, learning_rate=0.05, 
                                   subsample=0.8, colsample_bytree=0.8, random_state=42, verbosity=-1)
    meta_model.fit(meta_Xfull, y_raw_full)
    print(f"  Meta-learner (LightGBM) trained.")
    ...
    def tree_pred(X, raw_tabular_features):
        raw = np.column_stack([
            inv_log(xgbm.predict(X)), inv_log(lgbm.predict(X)),
            inv_log(gbrm.predict(X)), inv_log(catm.predict(X)),
            inv_log(etrm.predict(X))])
        meta_features = raw_tabular_features[:, meta_idx]
        meta_X = np.column_stack([raw, meta_features])
        return meta_model.predict(meta_X)

    tree_te = tree_pred(Xte, Xte)
    tree_va = meta_model.predict(meta_Xfull[len(Xtr):])
    xgb_va  = inv_log(xgbm.predict(Xva)); xgb_te  = inv_log(xgbm.predict(Xte))
    ...
    summary = {
        ...
        "elasticnet_weights": {"xgb": float(meta_model.feature_importances_[0]),
                               "lgb": float(meta_model.feature_importances_[1]),
                               "gbr": float(meta_model.feature_importances_[2]),
                               "cat": float(meta_model.feature_importances_[3]),
                               "etr": float(meta_model.feature_importances_[4])},
