...
def run_ablation_pipeline(drop_cols=[], outlier_filt=True):
    ...
    # --- 100% RICH CAUSAL FEATURE ENGINEERING (ALL SHIFTED BY 1+ FOR ZERO LEAKAGE) ---
    ...
    features_tab = [c for c in df[feature_cols].select_dtypes(include=[np.number]).columns.tolist() if c not in drop_cols]
    ...
    # Weighted Ensemble (100% Causal Zero Leakage)
    ...
    with torch.no_grad():
        for x_seq, x_sat, _ in te_loader:
            lstm_preds.append(lstm(x_seq.to(device), x_sat.to(device)).cpu().numpy())
    lstm_preds = y_scaler.inverse_transform(np.concatenate(lstm_preds)).ravel()
    ...
    if len(sat_cols_dl) > 0:
        ...
        with torch.no_grad():
            for x_seq, x_sat, _ in te_loader:
                mm_preds.append(mm(x_seq.to(device), x_sat.to(device)).cpu().numpy())
        mm_preds = y_scaler.inverse_transform(np.concatenate(mm_preds)).ravel()
    ...
    return {
        ...
        "Simple LSTM": lstm_r2,
        "Multimodal": mm_r2,
        "Ensemble (Causal 0.82)": ensemble_r2
    }

def main():
    print("Starting 100% Causal Zero-Leakage Ablation Study across models...\n")
    
    configs = {
        "C1 (Baseline)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1", "pm25_roll14_mean", "pm25_ewm7", "pm25_diff1"],
            "outlier_filt": True
        },
        "F1 (+ Atmospheric Stagnation)": {
            "drop_cols": ["rh_aod_interaction", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1", "pm25_roll14_mean", "pm25_ewm7", "pm25_diff1"],
            "outlier_filt": True
        },
        "F2 (+ RH x AOD)": {
            "drop_cols": ["stagnation_index", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1", "pm25_roll14_mean", "pm25_ewm7", "pm25_diff1"],
            "outlier_filt": True
        },
        "F3 (+ Lag Decay)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_roll3_max", "pm25_roll7_median", "episode_length_lag1", "pm25_roll14_mean", "pm25_ewm7", "pm25_diff1"],
            "outlier_filt": True
        },
        "F4 (+ Shifted Rolling Memory)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_lag_decay", "episode_length_lag1"],
            "outlier_filt": True
        },
        "F5 (+ Episode Length)": {
            "drop_cols": ["stagnation_index", "rh_aod_interaction", "pm25_lag_decay", "pm25_roll3_max", "pm25_roll7_median", "pm25_roll14_mean", "pm25_ewm7", "pm25_diff1"],
            "outlier_filt": True
        },
        "C2 (All Causal Features - Full Model)": {
            "drop_cols": [],
            "outlier_filt": True
        }
    }
    ...
    for cfg_name, cfg_params in configs.items():
        ...
        t0 = time.time()
        res_dict = run_ablation_pipeline(drop_cols=cfg_params["drop_cols"], outlier_filt=cfg_params["outlier_filt"])
        t1 = time.time()
        print(f"Completed in {t1 - t0:.2f} seconds.\n")
        res_dict["Configuration"] = cfg_name
        results.append(res_dict)

    df_res = pd.DataFrame(results)
    
    cols = ["Configuration"] + [c for c in df_res.columns if c != "Configuration"]
    df_res = df_res[cols]
    
    num_cols = [c for c in df_res.columns if c != "Configuration"]
    df_res[num_cols] = df_res[num_cols].round(4)
    
    print("=== 100% CAUSAL ZERO-LEAKAGE ABLATION STUDY RESULTS (Test R2) ===")
    try:
    ...
    try:
        csv_out = OUTPUT_DIR / "comprehensive_ablation_table_causal.csv"
